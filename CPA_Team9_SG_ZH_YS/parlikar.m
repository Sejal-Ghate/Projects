function [Cn, final_calibration_factor] = parlikar(hours, subj_name)
    ...
    Pdias = r_feat(:,4);
    PP = r_feat(:,5);
    ABP_Onset = abp_hrs(onset_times); % ABP at onsets
    delta_P = diff(ABP_Onset); % beat-to-beat pressure change at onset times
    PP = delta_P/(125*60);
    
    [co_uncal, to_par, told_par, fea_par, taus_tpr] = estimateCO_v3_Parlikar(onset_times,r_feat,beat_q,14,0);
...
%% Calibration of estimated co values from estimate function
    ...
    index_time = find(to_s == tco1_time(1)); %the index of the first co_uncalibrated from co when the first tco occurs in n_data
   ...
   Cn = tco1./est_co_uncalibrated(1); %get calibration factor k=first cotd measured/the corresponding uncalibrated estimated co at the same time
